...
public class LocalDateTimeConverter implements Converter {
	...
	public Object unmarshal(HierarchicalStreamReader reader, UnmarshallingContext context) {
		...
		long tempoEmMilis = Long.parseLong(milis); // conversão de variaveis
		Instant tempo = Instant.ofEpochMilli(tempoEmMilis);
		 
		ZoneId zone = ZoneId.of(timeZone); // conversão de variaveis
		ZonedDateTime dataComZona = ZonedDateTime.ofInstant(tempo, zone);
		
		LocalDateTime data = dataComZona.toLocalDateTime(); //conversão de metodos
